...
{ 
    ...
    if (!InitScript.losingLifeEveryGame && InitScript.lifes < InitScript.Instance.CapOfLife)
    ...
    var stats = new List<PlayFab.ClientModels.StatisticUpdate> {
        new PlayFab.ClientModels.StatisticUpdate { StatisticName = "HighScore", Value = Score },
        new PlayFab.ClientModels.StatisticUpdate { StatisticName = "Level", Value = currentLevel + 1 }
    };
    var request = new PlayFab.ClientModels.UpdatePlayerStatisticsRequest { Statistics = stats };
    PlayFab.PlayFabClientAPI.UpdatePlayerStatistics(request,
        result => Debug.Log("✅ PlayFab stats updated"),
