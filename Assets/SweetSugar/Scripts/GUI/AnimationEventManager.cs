...
//            LevelManager.THIS.gameStatus = GameState.Map;
        ...
        public async void BuyLife(GameObject button)
{
    SoundBase.Instance.PlayOneShot(SoundBase.Instance.click);

    Debug.Log("Sending Solana transaction to buy life...");

    try
    {
        // Send the Solana transaction using your LifeShop logic
        // bool success = await LifeShop.Instance.SendBuyLifeTransaction();
bool success = await LifeShop.Instance.SendBuyLifeTransaction();

        if (success)
        {
            Debug.Log("Solana transaction confirmed, adding life...");
            InitScript.Instance.RestoreLifes(); // Fully refill
            CloseMenu();
        }
        else
        {
            Debug.LogWarning("Solana transaction failed or was cancelled.");
        }
    }
    catch (Exception e)
    {
        Debug.LogError("BuyLife failed: " + e.Message);
    }
}
